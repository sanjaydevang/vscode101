...
def find_risk_factor_text(soup):
    start_pattern = re.compile(r"item\s*1a[\s\.:-]*risk factors", re.IGNORECASE)
    end_pattern = re.compile(r"item\s*1b[\s\.:-]*unresolved staff comments", re.IGNORECASE)
    ...
    for td in soup.find_all('td'):
        ...
        if start_pattern.search(text) and not start_tag:
            start_tag = td
        if end_pattern.search(text) and not end_tag:
            end_tag = td
    ...
    if start_tag and end_tag:
        ...
        while current and current != end_tag:
            if current.name == 'td':
                ...
                if 20 < len(content) < 300 and content.endswith('.'):
...
def main():
    ...
    output_csv_path = "rasamplemini_rfdtitle_completed.csv"
    output_rows = []
