                ...
                if filing_type == "10-K" and filing_date.startswith(str(filing_year)):
                    ...
                    if filing_link:
                        ...
                        if detail_response.status_code == 200:
                            ...
                            table = detail_soup.find('table', class_='tableFile', summary="Document Format Files")
                            if table:
                                for row in table.find_all('tr')[1:]:
                                    ...
                                    if len(cells) >= 3:
                                        if '10-K' in cells[1].text:
                                            ...
                                            if doc_link:
                                                final_url = "https://www.sec.gov" + doc_link['href']
                                                return final_url, filing_date
        return None, None
    except Exception as e:
        print(f"Error fetching 10-K for CIK {cik}: {e}")
        return None, None
...
def extract_risk_factor_titles_from_html(soup, cik=None, filing_year=None):
    ...
    for elem in section_elems:
        for tag in elem.find_all(['b', 'strong', 'i']):
            ...
            if 20 <= len(text) <= 250 and text.endswith('.'):
                titles.append(text)
        for p in elem.find_all('p'):
            text = p.get_text(strip=True)
            if (20 <= len(text) <= 250 and text.endswith('.') and
