...
HEADERS = {
    'User-Agent': 'YourEmail@example.com',  # Replace with your email
...
def extract_reporting_date(text, filing_year):
    ...
    # Try to find specific dates for well-known companies
    if '1750' in text[:1000]:  # Check if this is CIK 1750
        # Based on your sample, CIK 1750 has a fiscal year end of May 31
...
def find_risk_factor_section(soup):
    ...
    Returns:
        str: The HTML content of the risk factor section
    """
...
def extract_risk_factor_titles(section_soup):
    ...
    # This is the original method used in the provided code
...
def main():
    ...
    """
    # Update this to your input CSV file name
        ...
        for index, row in df.iterrows():
            ...
            for title in risk_factor_titles:
                ...
                output_rows.append({
                    ...
                    'filingdate': datetime.strptime(filing_date, '%Y-%m-%d').strftime('%m/%d/%Y') if '-' in filing_date else filing_date,
    ...
    if output_rows:
        ...
        print(f"\nExtraction completed. Output saved to {output_csv_path}")
    else:
